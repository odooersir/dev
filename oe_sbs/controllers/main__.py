...
class DocumentSBSController(http.Controller):
    ...
    def send_to_sbs(self, document_ids):
        ...
        for document in documents:
            ...
            # Skip split parents: their sheets are already separate single-sheet docs
            if document.sbs_split_done:
                results.append({
                    'document_id': document.id,
                    'document_name': document.name,
                    'status': 'no',
                    'error': 'This multi-sheet file was split into separate '
                             'single-sheet documents. Please send those instead.',
                })
                continue
            ...
            if result.get('status') == 'ok':
                ...
                document.sudo().write({'import_number': result.get('import_number'),'template_id': result.get('template_id'),'modifier': request.env.user,'state': 'sent_to_sbs','tag_ids': [(4, tag.id) for tag in tags_to_add]})
    ...
    def delete_from_sbs(self, document_ids):
        ...
        for document in documents:
            ...
            if result.get('status') == 'ok':
                ...
                document.sudo().write({'import_number': False,'template_id': False,'modifier': request.env.user,'state':'deleted_from_sbs','tag_ids':[(4, tag.id) for tag in tags_to_add]})
                #tags_to_add = request.env['documents.tag'].search([('state', '=', 'sent_to_sbs')])
                #document.tag_ids = [(4, tag.id) for tag in tags_to_add]
            else:
                result_summary.update({
